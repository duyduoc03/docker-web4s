...
<table class="table mb-0">
	...
	{foreach from = $mutiple_language_all key = lang item = item_mutiple}
		...
		{***seo_keyword*}
		{assign var = seo_keyword_before value = ''}
		{if !empty($item_mutiple['seo_keyword']['before'])}
			{$seo_keyword_before = $item_mutiple['seo_keyword']['before']}
		{/if}
		{assign var = seo_keyword_after value = ''}
		{if !empty($item_mutiple['seo_keyword']['after'])}
			{$seo_keyword_after = $item_mutiple['seo_keyword']['after']}
		{/if}


		{if $seo_keyword_before != $seo_keyword_after}
			<tr>
				<td class="kt-font-bolder">
					<img src="/admin/assets/media/flags/{$lang}.svg" style="width: 20px; height: 15px;"> {__d('admin', 'tu_khoa_seo')}
				</td>
				<td nh-log-field="before">
					{$seo_keyword_before}
				</td>
	...
	{/foreach}
		...
		{assign var = items_before value = []}
		{if !empty($before_change.items)}
			{$items_before = $before_change.items}
		{/if}

		{assign var = items_after value = []}
		{if !empty($after_change.items)}
			{$items_after = $after_change.items}
		{/if}

		{assign var = items_format value = []}
		{if !empty($items_before) || !empty($items_after)}
			{$items_format = $this->ProductAdmin->formatDataItemProduct($items_before, $items_after)}
		{/if}
		
		{if $items_before != $items_after && !empty($items_format)}

		 	{foreach from = $items_format key = code item = item_product}
		 		{foreach from = $item_product key = code item = item}
				<tr>	
					{if $item@first}
						<td class="kt-font-bolder" rowspan="{count($item_product)}">
							{__d('admin', 'phien_ban_san_pham')} 
						</td>
					{/if}

					{if $item['before']['value'] != $item['after']['value']}	
						<td class="json-content">
							{if !empty($item['before']['name'])}
						 			<strong>
							 			{$item['before']['name']}
							 		</strong>							 	
						 	{/if}
		
							{if !empty($item['before']['value'])}
							 	{foreach from = $item['before']['value'] key = code item = value_before}
							 		<p nh-log-field="before"> 
							 			{$value_before}
							 		</p>
							 	{/foreach}
							{/if}
					...
					{if $item['before']['value'] != $item['after']['value']}	
						<td  class="json-content">
							...
							{if !empty($item['after']['value'])}
							 	{foreach from = $item['after']['value'] key = code item = value_after}
							 		<p nh-log-field="after">
							 			{$value_after}
							 		</p>
							 	{/foreach}
							{/if}
		...
		{assign var = width_before value = "{if !empty($before_change.width)}{$before_change.width}{/if}"}
		{assign var = width_after value = "{if !empty($after_change.width)}{$after_change.width}{/if}"}
		{assign var = width_unit_before value = "{if !empty($width_before)}{$before_change.width_unit}{/if}"}
		{assign var = width_unit_after value = "{if !empty($width_after)}{$after_change.width_unit}{/if}"}
		
		{if $width_before != $width_after || $width_unit_before != $width_unit_after}
			<tr>
				<td class="kt-font-bolder">
					{__d('admin', 'chieu_rong')}
				</td>
				<td nh-log-field="before">
					{$width_before} {$width_unit_before}
				</td>
				<td nh-log-field="after">
					{$width_after} {$width_unit_after}
				</td>
				<td>
					<i nh-btn="show-change-field" class="fa fa-exchange-alt fa cursor-p fs-14 text-primary"></i>
				</td>
		...
		{assign var = length_before value = "{if !empty($before_change.length)}{$before_change.length}{/if}"}
		{assign var = length_after value = "{if !empty($after_change.length)}{$after_change.length}{/if}"}
		{assign var = length_unit_before value = "{if !empty($length_before)}{$before_change.length_unit}{/if}"}
		{assign var = length_unit_after value = "{if !empty($length_after)}{$after_change.length_unit}{/if}"}
		
		{if $length_before != $length_after || $length_unit_before != $length_unit_after}
			<tr>
				<td class="kt-font-bolder">
					{__d('admin', 'chieu_dai')}
				</td>
				<td nh-log-field="before">
					{$length_before} {$length_unit_before}
				</td>
				<td nh-log-field="after">
					{$length_after} {$length_unit_after}
				</td>
				<td>
					<i nh-btn="show-change-field" class="fa fa-exchange-alt fa cursor-p fs-14 text-primary"></i>
				</td>
		...
		{assign var = height_before value = "{if !empty($before_change.height)}{$before_change.height}{/if}"}
		{assign var = height_after value = "{if !empty($after_change.height)}{$after_change.height}{/if}"}
		{assign var = height_unit_before value = "{if !empty($height_before)}{$before_change.height_unit}{/if}"}
		{assign var = height_unit_after value = "{if !empty($height_after)}{$after_change.height_unit}{/if}"}
		
		{if $height_before != $height_after || $height_unit_before != $height_unit_after}
			<tr>
				<td class="kt-font-bolder">
					{__d('admin', 'chieu_cao')}
				</td>
				<td nh-log-field="before">
					{$height_before} {$height_unit_before}
				</td>
				<td nh-log-field="after">
					{$height_after} {$height_unit_after}
				</td>
				<td>
					<i nh-btn="show-change-field" class="fa fa-exchange-alt fa cursor-p fs-14 text-primary"></i>
				</td>
		...
		{assign var = weight_before value = "{if !empty($before_change.weight)}{$before_change.weight}{/if}"}
		{assign var = weight_after value = "{if !empty($after_change.weight)}{$after_change.weight}{/if}"}
		{assign var = weight_unit_before value = "{if !empty($weight_before)}{$before_change.weight_unit}{/if}"}
		{assign var = weight_unit_after value = "{if !empty($weight_after)}{$after_change.weight_unit}{/if}"}
		
		{if $weight_before != $weight_after || $weight_unit_before != $weight_unit_after}
			<tr>
				<td class="kt-font-bolder">
					{__d('admin', 'can_nang')}
				</td>
				<td nh-log-field="before">
					{$weight_before} {$weight_unit_before}
				</td>
				<td nh-log-field="after">
					{$weight_after} {$weight_unit_after}
				</td>
				<td>
					<i nh-btn="show-change-field" class="fa fa-exchange-alt fa cursor-p fs-14 text-primary"></i>
				</td>
		...
		{if $catalogue_before != $catalogue_after}
			<tr>
				<td class="kt-font-bolder">
		...
		{assign var = brand_name_before value = "{if !empty($before_change.brand_name)}{$before_change.brand_name}{/if}"}
		{assign var = brand_name_after value = "{if !empty($after_change.brand_name)}{$after_change.brand_name}{/if}"}
		
		{if $brand_name_before != $brand_name_after}
			<tr>
				<td class="kt-font-bolder">
		...
		{assign var = attributes_before value = []}
		{if !empty($before_change.attributes)}
			{$attributes_before = $this->AttributeAdmin->formatDataAttributes($before_change.attributes , $lang_log, 'product')}
		{/if}

		{assign var = attributes_after value = []}
		{if !empty($after_change.attributes)}
			{$attributes_after = $this->AttributeAdmin->formatDataAttributes($after_change.attributes , $lang_log, 'product')}
		{/if}

		{assign var = attributes_all value = []}
		{if !empty($attributes_before) || !empty($attributes_after)}
			{$attributes_all = $this->AttributeAdmin->formatDataAttributesAll($attributes_before ,$attributes_after )}
		{/if}

		{if $attributes_before != $attributes_after}
			{foreach from = $attributes_all key = code item = attribute}
				<tr>	
					{if $attribute@first}
						<td class="kt-font-bolder" rowspan="{count($attributes_all)}">
							{__d('admin', 'thuoc_tinh_mo_rong')} 
						</td>
					{/if}

					{if $attribute['before']['value'] != $attribute['after']['value']}	
						<td class="json-content">
							{if !empty($attribute['before']['name'])}
						 			<strong>
							 			{$attribute['before']['name']}
							 		</strong>							 	
						 	{/if}
		
							{if !empty($attribute['before']['value'])}
							 	{foreach from = $attribute['before']['value'] key = code item = value_before}
							 		<p nh-log-field="before"> 
							 			{$value_before|truncate:400:" ..."}
							 		</p>
							 	{/foreach}
							{/if}
						</td>
					...
					{if $attribute['before']['value'] != $attribute['after']['value']}	
						<td  class="json-content">
							...
							{if !empty($attribute['after']['value'])}
							 	{foreach from = $attribute['after']['value'] key = code item = value_after}
							 		<p nh-log-field="after">
							 			{$value_after|truncate:400:" ..."}
							 		</p>
							 	{/foreach}
							{/if}
						</td>
