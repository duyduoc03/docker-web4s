        ...
        <div class="row wrap-album">
            <div class="col-xl-6 col-lg-6">
                <div class="wrap-files">
                    <input id="{$code}_{$index}_files" name="" data-name="files" value="{if !empty($album.files)}{htmlentities($album.files)}{/if}" type="hidden" input-attribute="{ALBUM_FILE}" input-attribute-code="{$code}"/>
                    <div class="list-files">
                        {assign var = files value = []}
                        {if !empty($album.files)}
                            {$files = $album.files|json_decode:1}
                        {/if}
            ...
            {assign var = url_select_files value = "{CDN_URL}/myfilemanager/?cross_domain=1&multiple=1&token={$access_key_upload}&field_id={$code}_{$index}_files&lang={LANGUAGE_ADMIN}"}
