...
{/if}
    <div nh-product="{if !empty($product.id)}{$product.id}{/if}" nh-product-item-id="{if !empty($product.items[0])}{$product.items[0].id}{/if}" nh-product-attribute-special="{if !empty($product.attributes_item_special)}{htmlentities($product.attributes_item_special|@json_encode)}{/if}" class="product-item wrp-effect-scale swiper-slide mb-0">
        {if $product.apply_special && !empty($product.items[0].time_end_special) && $product.items[0].time_end_special > $smarty.now}
            <div nh-countdown="{$product.items[0].time_end_special}">
                <div class="countdown mb-3">
                    <div class="countdown--item">
                        <div nh-count-down-item="day" class="countdown--number"></div>
                        <div class="countdown--label">d</div>
                    </div>
                    <div class="countdown--item">
                        <div nh-count-down-item="hour" class="countdown--number"></div>
                        <div class="countdown--label">h</div>
                    </div>
                    <div class="countdown--item">
                        <div nh-count-down-item="minute" class="countdown--number"></div>
                        <div class="countdown--label">m</div>
                    </div>
                    <div class="countdown--item">
                        <div nh-count-down-item="second" class="countdown--number"></div>
        ...
        <div class="inner-content text-center">
            {if !empty($product.name)}
                <h4 class="product-title">
                    <a href="{$this->Utilities->checkInternalUrl($product.url)}" class="h5 mb-0">
                        {$product.name|escape|truncate:50:" ..."}
                    </a>
                </h4>
            {/if}

            <div class="price mt-3 h5 font-weight-bold">                        
