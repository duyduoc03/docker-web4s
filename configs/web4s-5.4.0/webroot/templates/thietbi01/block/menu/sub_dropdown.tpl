...
<ul nh-toggle-element="{$parent_menu_code}" class="entry-menu dropdown">
	{foreach from = $data_sub_menu key = k_0 item = sub_menu}
		<li class="{if !empty($sub_menu.children)}has-child{/if} navigation__item">
			<a class="menu-link navigation__link" href="{if !empty($sub_menu.url)}{$this->Utilities->checkInternalUrl($sub_menu.url)}{else}/{/if}">
