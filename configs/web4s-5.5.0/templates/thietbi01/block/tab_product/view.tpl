{strip}
    ...
    <div class="title-tab d-flex flex-column flex-md-row justify-content-between align-items-center mb-5">
            ...
            <ul class="block-tab nav effect-nav">
                {foreach from = $data_block.tabs item = tab}
                    <li class="nav-item navigation__item">
                        <a nh-active-tab="{$tab@index}" class="nav-link navigation__link link_black{if $tab@first} active{/if}" data-toggle="tab" href="#{$tab_id}-{$tab@index}">
                            {if !empty($tab.name)}
                                {$tab.name}
                            {/if}
                        </a>
